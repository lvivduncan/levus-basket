...
// це дозволе нормально розширяти кошик, а не переписувати його постійно
{
			...
			item.addEventListener('click', e => {
				...
				if (e.target === btn) {
					...
					add(
						...
						btn.dataset.price,                        // загальна ціна
						1,                                        // кількість
						item.querySelector('.sizes select').value // розмір
					);
					// оновлюємо кошик
					view();
					issue();
	...
	if (document.querySelector('#button') !== null) {
		document.querySelector('#button').addEventListener('click', function () {
			add(
				...
				button.dataset.sizeprice,   // розмір
				button.dataset.optionname,  // додаткові опції (сума опцій)
				button.dataset.optionprice, // додаткові опції (сума опцій)
	...
	function add(name = '', price = '', result = 1, sizeName = '', sizePrice = '', optionName = '', optionPrice = '', colorName = '', colorPrice = '') {
		...
		if (result === 1) {
			// назва
			localStorage.getItem('name') !== null ? localStorage.name += `~${name}` : localStorage.name = `${name}`;

			// ціна
			localStorage.getItem('price') !== null ? localStorage.price += `~${price}` : localStorage.price = `${price}`;

			// кількість
			localStorage.getItem('result') !== null ? localStorage.result = +`${result}` + +localStorage.result : localStorage.result = `${result}`;

			// розміри
			localStorage.getItem('sizeName') !== null ? localStorage.sizeName += `~${sizeName}` : localStorage.sizeName = `${sizeName}`;
			localStorage.getItem('sizePrice') !== null ? localStorage.sizePrice += `~${sizePrice}` : localStorage.sizePrice = `${sizePrice}`;

			// опції
			localStorage.getItem('optionName') !== null ? localStorage.optionName += `~${optionName}` : localStorage.optionName = `${optionName}`;
			localStorage.getItem('optionPrice') !== null ? localStorage.optionPrice += `~${optionPrice}` : localStorage.optionPrice = `${optionPrice}`;

			// колір
			localStorage.getItem('colorName') !== null ? localStorage.colorName += `~${colorName}` : localStorage.colorName = `${colorName}`;
			localStorage.getItem('colorPrice') !== null ? localStorage.colorPrice += `~${colorPrice}` : localStorage.colorPrice = `${colorPrice}`;	

		} else {

			// назва (true)
			localStorage.getItem('name') !== null ? localStorage.name += `~${name}`.repeat(`${result}`) 
				: localStorage.name = `~${name}`.repeat(`${result}`);

			// ціна (true)
			localStorage.getItem('price') !== null ? localStorage.price += `~${price/result}`.repeat(`${result}`) 
				: localStorage.price = `~${price/result}`.repeat(`${result}`);

			// кількість
			localStorage.getItem('result') !== null ? localStorage.result = `${result}`*1 + localStorage.result*1 : localStorage.result = `${result}`;

			// розміри
			localStorage.getItem('sizeName') !== null ? localStorage.sizeName += `~${sizeName}`.repeat(`${result}`) 
				: `${sizeName}` !== null ? localStorage.sizeName = `~${sizeName}`.repeat(`${result}`) 
				: localStorage.sizeName = `${sizeName}`;

			localStorage.getItem('sizePrice') !== null ? localStorage.sizePrice += `~${sizePrice}`.repeat(`${result}`) 
				: `~${sizePrice}` !== null ? localStorage.sizePrice = `~${sizePrice}`.repeat(`${result}`)
				: localStorage.sizePrice = `${sizePrice}`;

			// опції
			localStorage.getItem('optionName') !== null ? localStorage.optionName += `~${optionName}`.repeat(`${result}`) 
				: `${optionName}` !== null ? localStorage.optionName = `~${optionName}`.repeat(`${result}`)
				: localStorage.optionName = `${optionName}`;

			localStorage.getItem('optionPrice') !== null ? localStorage.optionPrice += `~${optionPrice}`.repeat(`${result}`) 
				: `${optionPrice}` !== null ? localStorage.optionPrice = `~${optionPrice}`.repeat(`${result}`)
				: localStorage.optionPrice = `${optionPrice}`;

			// колір
			localStorage.getItem('colorName') !== null ? localStorage.colorName += `~${colorName}`.repeat(`${result}`) 
				: `${colorName}` !== null ? localStorage.colorName = `~${colorName}`.repeat(`${result}`)
				: localStorage.colorName = `${colorName}`;

			localStorage.getItem('colorPrice') !== null ? localStorage.colorPrice += `~${colorPrice}`.repeat(`${result}`) 
				:`${colorPrice}` !== null ?  localStorage.colorPrice = `~${colorPrice}`.repeat(`${result}`)
	...
	function remove(e) {
		...
		if (localStorage.getItem('result') == 1) {
			...
			if (e.target.nodeName === 'P') {
				...
				// localStorage.setItem('result', result.join('~'));
				localStorage.setItem('result', result - 1);
    ...
    function order(){
		...
		name.forEach((item, i) => {
			goods.push(
				`<br><b>${name[i]}</b> ${price[i]} грн. ${(sizeName[i]) ? `розмір: ${sizeName[i]}` : ``} ${optionName[i]} ${colorName[i]}`
			);
		});
...
/* 		$.fancybox.open(`
			<form>
				...
				<div id="issue-form">
					<p><input type="text" placeholder="ПІП" required><input type="text" placeholder="Телефон" required></p>
					<p><input type="text" placeholder="Імейл" required><input type="text" placeholder="Адреса" required></p>
	 ...
	 */
/* 	function msg(el){
		$.fancybox.open(`<div id="issue"> Товар <b>${el.dataset.name}</b> додано у кошик </div>`);
